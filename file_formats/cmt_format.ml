...
let saved_types = Local_store.s_ref []
let uids_deps : (dependency_kind * Uid.t * Uid.t) list DLS.key =
  Local_store.s_ref []
...
let add_saved_type b = DLS.set saved_types (b :: DLS.get saved_types)
let get_saved_types () = DLS.get saved_types
let set_saved_types l = DLS.set saved_types l

let record_declaration_dependency (rk, uid1, uid2) =
  if not (Uid.equal uid1 uid2) then
    DLS.set uids_deps ((rk, uid1, uid2) :: DLS.get uids_deps)
    ...
    Misc.output_to_file_via_temporary
       ...
       (fun temp_file_name oc ->
         ...
         let cmt = {
           ...
           cmt_annots;
           cmt_declaration_dependencies = DLS.get uids_deps;
